...
import type { DashboardRealmScope } from '@/features/dashboard/dashboard.types'
...
export function ClientDashboardViewsForm({ client }: { client: Client }) {
  ...
  async function handleCreate() {
    ...
    try {
      await createView({ clientIdInternal, body: { name: name.trim(), type, realmScope } }).unwrap()
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="space-y-2">
        {views.length === 0 ? (
          ...
          views.map((view) => (
            <div key={view.dashboardViewId} className="grid grid-cols-[1fr_8rem_8rem_2.5rem] items-center gap-3 rounded-lg border border-border p-3">
              <div>
